...
import { FileText, Clock, AlertCircle, CheckCircle, Loader2, MoreVertical } from 'lucide-react';
        ...
        >
          <div className="flex justify-between items-start mb-3">
            <div className={`p-2.5 rounded-lg ${
              doc.status === 'completed' ? 'bg-indigo-100 text-indigo-600' :
              doc.status === 'processing' ? 'bg-amber-100 text-amber-600' :
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex items-center space-x-1 ${
              ...
              {doc.status === 'completed' && <CheckCircle size={10} className="mr-1" />}
